...
def split_dataframe(df: pd.DataFrame, config: TrainConfig) -> tuple[pd.DataFrame, pd.DataFrame]:
    if config.session_split:
        return split_by_session(df, config.val_ratio)
    return split_by_rows(df, config.val_ratio)
