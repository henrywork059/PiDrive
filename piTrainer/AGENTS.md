...
- Keep the Preprocess page arranged around the `1 Auto` fast path and `2 Settings` custom path. Most users should be able to click the green `Auto Preprocess Active Data` button after loading sessions. The recommended/default/auto preprocess recipe must include one horizontal flip copy for every active frame, with steering inverted on the flipped copy. Optional filters/augmentation/save-maintenance controls should stay behind collapsed advanced sections.
