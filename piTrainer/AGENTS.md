...
- V9 Data page overlay refactors, green path curves, default Speed bar / Steering arc visibility, responsive 30%-transparent overlay text, and the no-repeated-text drawing rule.
...
- Keep the shared horizontal-flip label guard in use everywhere a flipped row may be trained, validated, predicted for review, or displayed. A row with `aug_flip_lr=True` must use the flipped image and the inverted steering label from `source_steering` or a matched unflipped `source_frame_id`; speed/throttle must remain unchanged. Do not add a training/validation/review path that bypasses this guard.
