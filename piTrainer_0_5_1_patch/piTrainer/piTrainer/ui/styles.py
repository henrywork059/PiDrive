...
def build_stylesheet(density: str = "comfortable") -> str:
    ...
    values = profiles.get(density, profiles["comfortable"])
    css = """
    QWidget {
    ...
    """
    for key, value in {
        "BASE_FONT": values["base_font"],
        "BANNER_TITLE": values["banner_title"],
        "BUTTON_MIN": values["button_min"],
        "NEXT_MIN": values["next_min"],
        "INPUT_MIN": values["input_min"],
        "MAIN_TAB_MIN": values["main_tab_min"],
        "WORKFLOW_TAB_MIN": values["workflow_tab_min"],
        "SCROLLBAR": values["scrollbar"],
    }.items():
