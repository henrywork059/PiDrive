...
## What changed in 0_1_13

- replaces the older fixed splitter layouts with **dockable page panels** across Marking, Training, Validation, and Export
- panels can now be **dragged, snapped, floated, and merged into tabs** within each workflow page
- splits each page into more function-focused panels instead of a few large fixed regions
- adds **Validation autoplay** so predicted frames can cycle automatically
- adds more **page shortcuts**, especially for Validation frame review and autoplay
- keeps the earlier validation source-resolution fix from `0_1_12`
...
- uses **dockable panels** for source / sessions / images / classes / tools / current item
- supports multi-select frames with `Ctrl + Click`
- uses `A / D` for previous / next frame on the canvas
- uses `Alt + Left / Alt + Right` for page-level previous / next frame
...
- uses dockable panels for config / actions / status / preview / notes
...
- supports **autoplay** for saved prediction frames
- supports output styling controls for predicted boxes and labels
- includes a dedicated **Run Log**
- uses dockable panels for config / overlay settings / actions / status / browser / notes
...
- includes a dedicated **Run Log**
- uses dockable panels for config / actions / status / notes
...
- UI state is stored locally so the app can remember the last sessions root and the dock layout for each page.
- Press **F1** inside the app to view global and current-tab shortcuts.
