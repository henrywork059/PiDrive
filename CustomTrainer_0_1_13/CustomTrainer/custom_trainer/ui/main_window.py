...
class CustomTrainerMainWindow(QMainWindow):
    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle('CustomTrainer 0_1_13')
    ...
    def show_shortcuts(self) -> None:
        ...
        page_lines = []
        if hasattr(current_page, 'shortcut_lines'):
            try:
                page_lines = list(current_page.shortcut_lines())
            except Exception:
                page_lines = []
        lines = [
            'Global: Ctrl+1..Ctrl+4 switch tabs',
            'Global: Ctrl+Tab / Ctrl+Shift+Tab next / previous tab',
            'Global: F1 show shortcuts',
            'Global: Ctrl+S save current labels on Marking',
            'Global: PageUp / PageDown previous / next Marking frame',
            'Panels: drag any panel title bar to snap it to an edge or merge it into a tab with another panel',
            'Panels: Ctrl+Shift+0 resets the current page panel layout',
            'Canvas: right-click selects a box, left-drag creates a box',
            'Canvas: Arrow keys move selected box, Shift+Arrow moves faster',
            'Canvas: A / D previous / next frame, X delete selected frame(s), Backspace/Delete remove selected box',
        ]
        if page_lines:
            lines.extend(['', 'Current tab:', *page_lines])
