...
from custom_trainer.services.ui_state_service import get_last_sessions_root, set_last_sessions_root
...
class MarkingPage(DockWorkspacePage):
    ...
    def _build(self) -> None:
        ...
        reset_layout_button = QPushButton('Reset Panels', source_buttons)
        ...
        source_buttons_layout.addWidget(reset_layout_button)
        ...
        help_label = QLabel(
            """Drag panel title bars to snap, re-dock, or merge them into tabs.
...
Delete selected box: Backspace / Delete""",
        ...
        self.set_central_panel(preview_box, title='Image Preview')
        self.source_dock = self.create_dock('marking_source_dock', 'Session Source', source_box, area=Qt.LeftDockWidgetArea)
        self.sessions_dock = self.create_dock('marking_sessions_dock', 'Sessions', sessions_box, area=Qt.LeftDockWidgetArea)
        self.images_dock = self.create_dock('marking_images_dock', 'Images', images_box, area=Qt.LeftDockWidgetArea)
        self.classes_dock = self.create_dock('marking_classes_dock', 'Classes', classes_box, area=Qt.RightDockWidgetArea)
        self.tools_dock = self.create_dock('marking_tools_dock', 'Annotation Tools', tools_box, area=Qt.RightDockWidgetArea)
        self.info_dock = self.create_dock('marking_info_dock', 'Current Item', info_box, area=Qt.RightDockWidgetArea)
    ...
    def _apply_default_layout(self) -> None:
        ...
        self.workspace.addDockWidget(Qt.LeftDockWidgetArea, self.source_dock)
        self.workspace.splitDockWidget(self.source_dock, self.sessions_dock, Qt.Vertical)
        self.workspace.splitDockWidget(self.sessions_dock, self.images_dock, Qt.Vertical)
        self.workspace.addDockWidget(Qt.RightDockWidgetArea, self.classes_dock)
        self.workspace.splitDockWidget(self.classes_dock, self.tools_dock, Qt.Vertical)
        self.workspace.splitDockWidget(self.tools_dock, self.info_dock, Qt.Vertical)
        self.workspace.resizeDocks([self.source_dock, self.sessions_dock, self.images_dock], [120, 260, 420], Qt.Vertical)
        self.workspace.resizeDocks([self.classes_dock, self.tools_dock, self.info_dock], [340, 260, 220], Qt.Vertical)

    def _setup_shortcuts(self) -> None:
        shortcuts: list[tuple[str, Callable[[], None]]] = [
            ('X', self.delete_selected_frames),
            ('Ctrl+R', self.scan_sessions),
            ('Ctrl+L', lambda: self.restore_last_sessions_root(auto_scan=True)),
            ('Ctrl+Shift+S', self.save_current_labels),
            ('Ctrl+Shift+0', self.reset_panel_layout),
            ('Alt+Left', self.prev_image),
            ('Alt+Right', self.next_image),
        ]
    ...
    def shortcut_lines(self) -> list[str]:
        return [
            'Marking: Ctrl+R scan sessions',
            'Marking: Ctrl+L load last sessions root',
            'Marking: Ctrl+Shift+S save labels',
            'Marking: Alt+Left / Alt+Right previous / next frame',
            'Marking: Ctrl+Shift+0 reset dock layout',
            'Canvas: A / D previous / next frame, X delete selected frame(s)',
        ]
