...
DEFAULT_PERCEPTION_SETTINGS: dict[str, Any] = {
    ...
    "labels": {
        # Defaults are intentionally easy to retune in runtime_settings.json.
        # he3 defaults to orange/yellow-ish objects.
        "he3": {
            "ranges": [
        ...
        # he3_zone defaults to blue markers.
        "he3_zone": {
            "ranges": [
...
def merge_perception_settings(data: dict[str, Any] | None) -> dict[str, Any]:
    return _deep_merge(DEFAULT_PERCEPTION_SETTINGS, data or {})


def perception_backend_ready() -> tuple[bool, str]:
    if cv2 is None:
...
def build_frame_perception(frame_bgr, settings: dict[str, Any] | None = None) -> FramePerception:
    ...
    ready, _ = perception_backend_ready()
    if not ready or frame_bgr is None or frame_w <= 0 or frame_h <= 0:
        return perception
