...
class ExportValidatePage(QWidget):
    def __init__(self, state: AppState, log: Callable[[str], None], set_status: Callable[[str], None], parent: QWidget | None = None) -> None:
        ...
        self.val_button = QPushButton('Run Exported Validation', self)
        self.predict_button = QPushButton('Run Exported Prediction', self)
        ...
        self.run_log_output.setPlaceholderText('Exported model validation / prediction runtime log will appear here...')
    ...
    def _build(self) -> None:
        config_box = QGroupBox('Exported Model Validation / Prediction Config', self)
        form = QFormLayout(config_box)
        form.addRow('Exported Model', self._path_row(self.weights_edit, self.choose_weights))
        ...
        latest_best_button = QPushButton('Use Latest Exported Model', actions_box)
        ...
        preview_box = QGroupBox('Exported Model Preview', self)
        ...
        notes_box = QGroupBox('Exported Model Notes', self)
        notes_text = QPlainTextEdit(self)
        notes_text.setReadOnly(True)
        notes_text.setPlainText(
            '1. Choose an exported model such as a .tflite or .onnx file.\n'
            '2. Use Current Session Frames to point prediction at the whole current images folder.\n'
            '3. Run Exported Validation for metrics or Run Exported Prediction for saved preview frames.\n'
            '4. Use Prev/Next Frame or the drop-down to inspect exported-model performance visually.'
        )
    ...
    def restore_splitters(self) -> None:
        for name, splitter in (
            ('export_validate_main_splitter', self.main_splitter),
            ('export_validate_preview_splitter', self.preview_splitter),
            ('export_validate_side_splitter', self.side_splitter),
        ):
    ...
    def refresh_devices(self, *, log_runtime: bool) -> None:
        ...
        if log_runtime:
            ...
            self.set_status('Device list refreshed for exported-model validation.')
    ...
    def choose_weights(self) -> None:
        path, _ = QFileDialog.getOpenFileName(
            self,
            'Choose exported model',
            filter='Exported Models (*.tflite *.onnx *.pt *.torchscript *.pb);;All Files (*)',
        )
        if path:
    ...
    def use_current_root_defaults(self) -> None:
        ...
        self.refresh_devices(log_runtime=False)
        self.refresh_preview()
        self.set_status('Exported-model defaults filled from the current session frames.')

    def use_latest_best_weights(self) -> None:
        best = self.state.latest_best_weights()
        if best is None:
            QMessageBox.information(self, 'No exported model found', 'Export a model first, then try again.')
            return
        self.weights_edit.setText(str(best))
        self.set_status('Loaded latest exported model into Export Validate.')
        self.refresh_preview()
    ...
    def refresh_preview(self) -> None:
        ...
        if not source_text:
            ...
            self.preview_info.setText('Pick an image or folder source, or use the current frame/session from Marking to test the exported model.')
            return
        ...
        info_lines = [
            ...
            f'Weights: {self.weights_edit.text().strip() or "not set"}',
            f'Dataset YAML: {self.yaml_edit.text().strip() or "not set"}',
            'Raw source preview. Run Exported Prediction to render boxed detections for all frames.',
        ]
        if source_note:
            info_lines.append(f'Resolved predict source: {resolved_source_path}')
        if preview_source_path.is_dir():
    ...
    def start_val(self) -> None:
        if self.thread is not None:
            QMessageBox.information(self, 'Busy', 'Another exported-model task is already running.')
            return
        try:
        ...
        if not self.weights_edit.text().strip():
            QMessageBox.critical(self, 'Missing inputs', 'Choose an exported model first.')
            return
        ...
        if not self.yaml_edit.text().strip():
            QMessageBox.critical(self, 'Missing inputs', 'Choose an exported model and dataset.yaml first.')
            return
        if dataset_summary is not None:
            ...
            if not dataset_summary.has_usable_labels:
                QMessageBox.critical(
                    self,
                    'No usable exported-validation labels found',
                ...
                return
        project_dir, run_name = self._prediction_output_args('export_val')
        ...
        self.last_metrics_text = ''
        self._launch(command, 'Exported-model validation started...')
    ...
    def start_predict(self) -> None:
        ...
        if not self.weights_edit.text().strip() or not source_text:
            QMessageBox.critical(self, 'Missing inputs', 'Choose an exported model and a prediction source first.')
            return
        source_path = Path(source_text)
        if not source_path.exists():
            ...
            return
        resolved_source_path, source_note = resolve_prediction_source(source_path)
        project_dir, run_name = self._prediction_output_args('export_predict')
        ...
        self._launch(command, 'Exported-model prediction started...')
    ...
    def stop_task(self) -> None:
        ...
        self.stop_button.setEnabled(False)
        self.status_note.setText('Stopping exported-model task...')
        self.set_status('Stopping exported-model task...')
    ...
    def _on_finished(self, exit_code: int) -> None:
        ...
        if self.current_task_kind == 'predict' and exit_code == 0:
            ...
            message = f'Exported-model validation finished. {self.last_metrics_text}'
        else:
            message = f'Exported-model task finished with exit code {exit_code}.'
